...
## Analysis
This is similar to the problem where we check whether a tree is balanced tree in O(n) time.
In that problem we calculate tree height and determine whether tree is balanced or not at the same, and we only need to traverse the tree one time.
