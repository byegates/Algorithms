...
3. [105. Construct Binary Tree from Preorder and Inorder Traversal](#105-Construct-Binary-Tree-from-Preorder-and-Inorder-Traversal)
...
pL: 2, pR: 1, iL: 0, iR: -1 (pL > pR, left tree has nothing, but we didn't exit, iR = -1 (& < iL) says the same thing)
Processing left  Subtree for node: 9:
  preorder:
  []
  inorder :
  []
We still use pL as root to get root idx and left subtree len which is completely wrong, with all these value we now get right len -4 (iR(-1) - root idx(3))
