 ...
 */
```
## Another way to write state machine, 1ms, 100%
```java
class Solution {
    public int maxProfit(int k, int[] prices) {
        ...
        for (int price: prices) {
            for (int j = 1; j <= k; j++) {
                balance[j] = Math.max(balance[j], profit[j - 1] - price); // 今天买 vs 今天 躺平
                 profit[j] = Math.max(profit[j], balance[j] + price); // 今天卖 vs 今天 躺平
            }
        }
        return profit[k];
    }
