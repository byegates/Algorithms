...
TC: 25 * c * L^2 * N ==> O(L^2*N), 25C*L^2 transformations to get all edges for one node,
...
## Solution 1
No string to Index map

从前往后存, 从end开始找Path, 最后reverse path
```java
class Solution {
...
## Solution 2
TLE on LeetCode, Good on LaiCode

从前往后寸，从前往后找, String to Index Map
```java
class Solution {
...
## Solution 3 Using a graphNode, create the real graph
反着存，最后的path要reverse
```java
class Solution {
