...
class Solution {
   ...
   private void dfs(int cost, int cur, int dst, int k, List<Edge>[] graph, boolean[] visited) {
      if (cur == dst) {
      ...
      for (Edge e : graph[cur]) {
         ...
         visited[e.id] = true;
         dfs(cost + e.cost, e.id, dst, k - 1, graph, visited);
         visited[e.id] = false;
      }
   }
}
```
### 2d De-dup (69ms, 10.59%)
```java
class Solution {
   record Edge (int id, int cost) {}
   private int res;
   ...
   private void dfs(int cost, int cur, int dst, int k, List<Edge>[] graph, Integer[][] dp) {
      if (cur == dst) {
         if (cost < res) res = cost;
         return;
      }
      ...
      for (Edge e : graph[cur]) {
         ...
         if (dp[k][e.id] != null && newCost >= dp[k][e.id]) continue;
         dp[k][e.id] = newCost;
         dfs(newCost, e.id, dst, k - 1, graph, dp);
      }
   }
}
```
...
class Solution {
   ...
   public int findCheapestPrice(int n, int[][] flights, int src, int dst, int k) {
      ...
      PriorityQueue<Node> q = new PriorityQueue<>((a, b) -> (a.cost == b.cost ? a.steps - b.steps : a.cost - b.cost));
      q.offer(new Node(src, 0, 0));
      int[] stops = new int[n];
      ...
      while (!q.isEmpty()) {
         ...
         if (cur.steps >= stops[cur.id]) continue;
         stops[cur.id] = cur.steps;

         for (Edge next : graph[cur.id])
            if (cur.steps + 1 < stops[next.id])
               q.offer(new Node(next.id, cur.cost + next.cost, cur.steps + 1));
      }
...
```
## Solution 3b, 融合了LeetCode官方答案的改进, 6ms, 86.1%
