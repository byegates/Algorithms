...
4. [1168. Optimize Water Distribution in a Village](#1168_Optimize_Water_Distribution_in_a_Village)
   1. [Kruskal, PriorityQueue, O(V^2log(V)), 28ms, 100%](1168_Kruskal)
...
class Solution {
    ...
    private int minWell(int n, int[] wells) {
        int minWell = -1, minCost = Integer.MAX_VALUE;
        for (int i = 0; i < n; i++) if (wells[i] < minCost) {
            minCost = wells[i];
            minWell = i;
        }
